package kr.co.from.common.file;
...
public class FileService {
	...
	private String pattenDate = new String("yyyy/MM/dd");
	...
	public BoardFileDTO uploadSingleFile(MultipartFile multipartFile, HttpSession httpSession){
		...
		if(multipartFile != null 
			...
			if(limitFileSize < filesize){
				fileInfo.setErrorCode(-1); 		// 에러
				return fileInfo;
			}
			
			// 저장경로
			String defaultPath = httpSession.getServletContext().getRealPath("/"); //서버기본경로 (프로젝트 폴더 아님)
			String uriPath = sfPathName.format(new Date());
			//String uriPath = "upload/" +  sfPathName.format(new Date());
			
			//원본 파일 저장경로 처리
			String FileOrgPath = "upload/";
			String path = defaultPath + FileOrgPath + uriPath;
			
			makePath(path);
			
			//이미지 섬네일 저장경로 처리
			String thumbOrgPath = "resources/img/thumb/";
			String thumbPath = defaultPath + thumbOrgPath + uriPath;
			...
			//저장명 처리2
			fileInfo.setOrgFileNm(orgFileName);
			fileInfo.setNewFileNm(newFileName);
			fileInfo.setFpath(uriPath);
			fileInfo.setExt(ext);
			fileInfo.setFsize(multipartFile.getSize());
			...
			try{
				...
				logger.debug("=========> file type : " + fileType.substring(0, 5));

				//첨부파일이 이미지일 경우 섬네일 생성 업로드
				if (fileType.substring(0, 5).equals("image")) {
					File thumbNail =  new File(thumbPath + File.separator + newFileName);
					logger.debug("===================>" + thumbPath);
									
					if (file.exists()) {
						Thumbnails.of(file).size(200, 200).outputFormat(ext).toFile(thumbNail);
					}
	...
	public void downloadFile(HttpServletResponse response, HttpSession httpSession, BoardFileDTO fileDTO) {
		...
		try {
			...
			File file = new File(httpSession.getServletContext().getRealPath("/")+"upload/" + fileDTO.getFpath() + "/" +  fileDTO.getNewFileNm() );
			...
			String fileName = encodeFileName( fileDTO.getOrgFileNm() );
